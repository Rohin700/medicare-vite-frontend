...
const EquipmentFinder = () => {
  ...
  const handleFetch = async () => {
    if (!selectedCity || !equipmentName.trim()) {
      alert('Please select a city and enter a equipment name');
      return;
    }
    ...
    try {
      let url = server_url + "/recipient/dofetchequip";
      let fd = new FormData();

      fd.append("city" , selectedCity);
      fd.append("equipment" , equipmentName.toLowerCase());
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Find equipments and their availability in your city</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="Enter equipment name (e.g., wheerchair, stethoscope)"
        ...
        {searchResults && searchResults.length > 0 ? (
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              Found {searchResults.length} result{searchResults.length !== 1 ? 's' : ''} for "{equipmentName}" in {selectedCity}
            </h3>
                ...
                <div key={index} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-wrap items-center gap-2">
                      ...
                      <h4 className="text-lg font-semibold text-gray-600 capitalize">
                        Purchase Date : {result.purchasedtime?.substring(0,10)}
                      </h4>
                    </div>
                  </div>
          ...
          searchResults && searchResults.length === 0 && selectedCity && equipmentName && !loading && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-yellow-800">No results found for "{equipmentName}" in {selectedCity}.</p>
            </div>
          )
        )}
      </div>
